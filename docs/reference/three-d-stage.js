 ...
 *
 * three.js loads through the page's import map. The starter version of this
 * file mandates a pinned unpkg map with integrity hashes; **this repo
 * deliberately does not use it**, because a reference build that needs a CDN
 * cannot be rendered where egress is filtered — which is most of the sessions
 * that work on this board — and because a reference on a different three.js
 * version to `src/` can disagree with the rig for reasons neither file records.
 *
 * Reference pages here point at the copy in `node_modules` instead, served by
 * Vite from the repo root, which is what the leading `/` means:
 *
 ...
 *   </script>
 *
 * Re-copying this file from the starter restores the unpkg map in this comment
 * and will not touch the pages, so check `docs/reference/README.md` rather than
